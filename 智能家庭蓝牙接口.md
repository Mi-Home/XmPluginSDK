...
如果要获取BluetoothGatt或Service UUID列表，可以在connect时传入BleConnectResponse2，如下：
...
如果希望蓝牙写时带WRITE_TYPE_NO_RESPONSE标志，则传BleWriteResponse2回调。
...
状态通知包括蓝牙连接状态变化通知、character写通知和notify通知，都是通过BroadcastReceiver实现的，如果要关注相关的状态，则需要注册Receiver，如下：
...
private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (XmBluetoothManager.ACTION_CHARACTER_CHANGED
			...
			int status = intent.getIntExtra(
				XmBluetoothManager.KEY_CHARACTER_WRITE_STATUS,
				XmBluetoothManager.STATUS_UNKNOWN);
